...
WEB_ALLOWED_VIDEO_EXTENSIONS = set(ALLOWED_VIDEO_EXTENSIONS) | {".mts"}
...
async def list_jobs(request: Request) -> list[dict[str, object]]:
    return _runtime(request).store.list_jobs()
...
async def create_job(
    ...
    if suffix not in WEB_ALLOWED_VIDEO_EXTENSIONS:
    ...
    try:
        ...
        await video.seek(0)
        bytes_written = save_upload_file(
            video.file,
            upload_path,
...
async def retry_enhancement(job_id: str, request: Request) -> dict[str, object]:
    try:
        job = _runtime(request).store.retry_enhancement(job_id)
