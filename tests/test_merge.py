...
class MergeHelpersTest(unittest.TestCase):
    def test_align_segments_includes_unmatched_blocks(self) -> None:
        ...
        srt_segments = [(0.0, 1.0, "srt one"), (5.0, 6.0, "srt three")]
    ...
    def test_align_segments_allows_one_srt_to_cover_multiple_asr_blocks(self) -> None:
        asr_segments = [(0.0, 2.0, "Hello"), (2.0, 4.0, "world")]
        srt_segments = [(0.0, 4.0, "Hello world")]

        aligned = align_segments(asr_segments, srt_segments)

        self.assertEqual([block.srt_text for block in aligned], ["Hello world", "Hello world"])
        self.assertEqual(merge_blocks_prefer_srt(aligned), [(0.0, 2.0, "Hello world"), (2.0, 4.0, "Hello world")])
